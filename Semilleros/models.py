...
class Semillero(models.Model):
    ...
    docente_investigador = models.CharField(
        max_length=2, choices=DOCENTE_INVESTIGADOR, default="si"
    )
    ...
    tiene_proyectos = models.CharField(
        max_length=2, choices=TIENE_PROYECTOS_CHOICES, default="no"
    )
...
class InscripcionSemillero(models.Model):
    ...
    participa_proyectos = models.CharField(
        max_length=2, choices=PARTICIPA_PROYECTOS, default="no"
    )
    ...
    proyecto_interes = models.ForeignKey(
        Proyecto, on_delete=models.SET_NULL, blank=True, null=True
    )

    linea_sublinea = models.CharField(max_length=100, blank=True)
    horas_semanales = models.PositiveIntegerField(
        default=1, validators=[MaxValueValidator(168)]
    )
    TIPO_SEMILLERO = (("unica", "Fase Unica"),)
...
class ActividadesSemillero(models.Model):
    ...
    participantes_actividad_e = models.PositiveIntegerField(
        default=0, validators=[MaxValueValidator(10)]
    )
    adjunto_actividad_e = models.FileField(
        upload_to="Adjuntos_Actividades_E/", blank=True, null=True
    )
