...
from Users.models import User, Usuario, Usuario_Estudiante

from .forms import (
    ActividadesSemilleroForm,
    BuscarEstudianteForm,
    InscripcionSemilleroForm,
    SemilleroForm,
)
from .models import ActividadesSemillero, InscripcionSemillero, Semillero
...
def crear_semillero(request):
    ...
    if (
        not usuario_actual.telefono
        or not usuario_actual.facultad
        or not usuario_actual.Programa
    ):
        messages.error(request, "Se requiere completar los datos de tu perfil.")
        return redirect("Users:Users_editar_perfil")

    if (
        not usuario_actual.grupo_investigacion
        and not usuario_actual.grupo_investigacion_2
        and not usuario_actual.grupo_investigacion_3
    ) or not usuario_actual.lineas_investigacion:
        messages.error(
            request,
            "Se requiere especificar minimo un Grupo y una Linea de investigacion",
        )
    ...
    if request.method == "POST":
        ...
        linea_investigacion = (
            perfil_usuario.lineas_investigacion.first().nombre
            if perfil_usuario.lineas_investigacion.exists()
            else None
        )
    ...
    return render(
        request,
        "Docentes/crear_semillero.html",
        context={"form": form, "proyectos": proyectos},
    )
...
def lista_semilleros(request):
    ...
    context = {"semilleros": semilleros, "success_message": success_message}
...
def detalle_semillero(request, semillero_id):
    ...
    return render(
        request,
        "Docentes/detalle_semillero.html",
        {
            "semillero": semillero,
            "proyectos": proyectos,
...
def ver_perfil_solicitante(request, estudiante_id, semillero_id):
    ...
    return render(
        request,
        "Docentes/ver_perfil_solicitante.html",
        {"estudiante": estudiante, "semillero": semillero},
    )
...
def generar_aval(request, estudiante_id, semillero_id):
    ...
    html = render_to_string(
        "Docentes/generar_aval_pdf.html",
        {"semillero": semillero, "estudiante": estudiante},
    )

    response = HttpResponse(content_type="application/pdf")
    response[
        "Content-Disposition"
    ] = 'attachment; filename="Carta Aval Semillero - Estudiante.pdf"'
...
def ver_actividades_semillero(request):
    ...
    return render(
        request, "Docentes/ver_actividades_semillero.html", {"actividades": actividades}
    )
...
def ver_detalle_actividad_semillero(request, actividad_id):
    ...
    return render(
        request,
        "Docentes/ver_detalle_actividad_semillero.html",
        {"actividad": actividad},
    )
...
def ver_semilleros_estudiante(request):
    ...
    return render(
        request,
        "Estudiantes/ver_semilleros_estudiante.html",
        {"semilleros_disponibles": semilleros_disponibles, "usuario_e": usuario_e},
    )
...
def ver_mis_semilleros(request):
    ...
    return render(
        request,
        "Estudiantes/ver_mis_semilleros.html",
        {"usuario_e": usuario_e, "semilleros_inscritos": semilleros_inscritos},
    )
...
def detalle_semillero_estudiante(request, semillero_id):
    ...
    existe_inscripcion = InscripcionSemillero.objects.filter(
        semillero=semillero, user_e=usuario_estudiante
    ).exists()

    if existe_inscripcion:
        datos_e = InscripcionSemillero.objects.get(
            semillero=semillero, user_e=usuario_estudiante
        )
        ...
        if request.method == "POST":
            ...
            if actividades_form.is_valid():
                ...
                if "adjunto_actividad_e" in request.FILES:
                    nueva_actividad.adjunto_actividad_e = request.FILES[
                        "adjunto_actividad_e"
                    ]
                    nueva_actividad.save()
                return redirect(
                    "Semilleros:detalle_semillero_estudiante", semillero_id=semillero.id
                )
            ...
            return render(
                request,
                "Estudiantes/detalle_semillero_estudiante.html",
                {
                    "semillero": semillero,
...
def formulario_inscripcion_semillero(request, semillero_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            tipo_solicitante_otro = (
                form.cleaned_data["tipo_solicitante_otro"]
                if tipo_solicitante == "otro"
                else ""
            )
    ...
    return render(
        request,
        "Estudiantes/formulario_inscripcion_semillero.html",
        {"semillero": semillero, "form": form, "proyectos_docente": proyectos_docente},
    )
...
def ver_semilleros_admin(request):
    ...
    return render(
        request, "AdminSemilleros/ver_semilleros_admin.html", {"semilleros": semilleros}
    )
...
def detalle_semillero_admin(request, semillero_id):
    ...
    return render(
        request,
        "AdminSemilleros/detalle_semillero_admin.html",
        {
            "semillero": semillero,
            "estudiantes_inscritos": estudiantes_inscritos,
            "proyectos_docente": proyectos_docente,
        },
    )
...
def informes_facultad(request):
    ...
    Total_Estudiantes_Inscritos = (
        Usuario_Estudiante.objects.filter(inscripcionsemillero__isnull=False)
        .distinct()
        .count
    )
    ...
    return render(
        request,
        "AdminSemilleros/informes_facultad.html",
        {
